...
resource "aws_subnet" "inspection_fw_subnet" {
  for_each          = { for inspection_fw_subnet in var.inspection_fw_subnet_configs : inspection_fw_subnet.az => inspection_fw_subnet }
  vpc_id            = aws_vpc.inspection-vpc.id
  cidr_block        = each.value.cidr
  availability_zone = inspection_fw.aws_availability_zones.available.names[each.value.az]
  tags = merge(var.tags,
    {
